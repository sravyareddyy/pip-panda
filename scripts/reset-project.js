 ...
 * It deletes or moves the /src and /scripts directories to /example based on user input and creates a new /src/app directory with an MicButton.tsx and layout.tsx file.
...
const moveDirectories = async (userInput) => {
  try {
    ...
    // Create MicButton.tsx
    const indexPath = path.join(newAppDirPath, "MicButton.tsx");
    await fs.promises.writeFile(indexPath, indexContent);
    console.log("📄 src/app/MicButton.tsx created.");

    // Create layout.tsx
    const layoutPath = path.join(newAppDirPath, "layout.tsx");
    await fs.promises.writeFile(layoutPath, layoutContent);
    console.log("📄 src/app/layout.tsx created.");
